...
//
// A database that is already fully migrated — with a complete pass recorded by
// the pass-completion sentinel (migrationPassCompleteKey), so no other
// process's pass tail can still be in flight — skips the lock and the
...
// serialize migrations, not the steady-state no-op check.
//
// Anything less than fully-current falls through to the locked pass, which
// re-checks under the lock (double-checked locking) and surfaces real errors.
// A probe ERROR also falls through rather than failing the open: the locked
// path is the authority, and a transient read failure must not turn a healthy
// open into a hard error.
